// ALEPH T04 공용 어댑터 로직.
// 참조 구현(adapter-reset.example.js)의 정규화·저장·비교 규칙을 그대로 옮긴 것으로,
// 실제 라이브 조회 경로(app/api/board/route.js)와 합성 재생 경로(components/ReplayPanel.jsx)가
// 같은 함수를 호출하도록 만들어 오류 처리 로직이 두 곳에서 따로 놀지 않게 한다.
...
export function kstDateFromISO(isoString) {
...
export function validateNormalizedReading(reading) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(reading.record_date) || reading.record_date !== kstDateFromISO(reading.fetched_at)) {
